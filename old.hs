...
ppInit :: PPState
ppInit = PPState 0 M.empty

--ppDefine :: PPState -> String -> (PPState, String)
--ppDefine state buffer =
--  let 
...
parser :: String -> Either Error (Statement, String)
...
parseNextToken :: Token -> String -> Either Error (Statement, String)
...
parseFunctionDeclaration :: Type -> String -> Either Error (Statement, String)
...
parseFunParametersEx params buffer =
  case getNextToken buffer of
    ...
    Right (Type Void, rest) -> do
        _ <- skipToken RPar rest
        return (params, rest)
    Right _ -> do
...
parseTypeDefinition :: String -> Either Error (Statement, String)
...
parseNewType :: String  -> Either Error (Statement, String)
...
parseTypeDeclaration :: String -> Either Error (Statement, String)
...
parseStructDefinition :: String -> Either Error (Statement, String)
parseStructDefinition buffer = do
  ...
  case _nextToken of
    ...
    tok -> (markError _b) $ Left $ "Unexpcted token " ++ show tok ++" in struct Statement."
...
parseEnumDefinition :: String -> Either Error (Statement, String) 
